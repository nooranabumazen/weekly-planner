...
const CATEGORY_KEYWORDS = {
  cat_work: ["meeting", "email", "report", "deadline", "presentation", "project", "client", "office", "review", "submit"],
  cat_personal: ["call", "visit", "birthday", "gift", "family", "friend", "party", "event", "dinner", "gathering"],
  cat_health: ["exercise", "gym", "run", "walk", "yoga", "stretch", "doctor", "dentist", "meditate", "sleep"],
  cat_errands: ["grocery", "groceries", "bank", "post office", "pharmacy", "pick up", "drop off", "appointment", "return", "buy"],
  cat_learning: ["learn", "study", "course", "read", "book", "lecture", "class", "homework", "research", "practice", "tutorial"],
  cat_selfcare: ["shower", "hair", "nails", "skincare"],
};
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "4px 16px", maxWidth: 500 }}>
        {categories.map((cat) => (
          <div key={cat.id} ref={editingId === cat.id ? (el) => { if (el) setTimeout(() => el.scrollIntoView({ behavior: "smooth", block: "nearest" }), 50); } : undefined} style={{ padding: "8px 0", borderBottom: "1px solid var(--border-light)" }}>
            {editingId === cat.id ? (
              <div>
                ...
                <div style={{ marginBottom: 4 }}>
                  ...
                  <textarea value={editKeywords} onChange={(e) => setEditKeywords(e.target.value)}
                    ...
                    style={{ width: "100%", border: "1px solid var(--border)", borderRadius: 4, padding: "5px 8px", fontSize: 12, outline: "none", background: "var(--bg-card)", color: "var(--text)", boxSizing: "border-box", resize: "none", overflow: "hidden", fontFamily: "'DM Sans', sans-serif", lineHeight: 1.5 }}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
          ...
          {dailyHabits.map((h) => {
            ...
            return (
              ...
              <div style={{ width: nameColWidth + 7 * 28 + 18, height: 0, borderBottom: "0.5px solid var(--border-light)", opacity: 0.5, marginBottom: 1 }} />
              </div>);
          })}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
          ...
          {weeklyHabits.map((h) => (
            ...
            <div draggable onDragStart={() => setDragHabit({ id: h.id, type: "weekly" })} onDragOver={(e) => e.preventDefault()} onDrop={() => handleWeeklyDrop(h.id)}
              ...
              {h.done && h.note && <span title={h.note} style={{ fontSize: 8, color: "var(--text-faint)", marginLeft: "auto", flexShrink: 0, cursor: "help", fontFamily: "'JetBrains Mono', monospace", background: "var(--border-light)", padding: "0 3px", borderRadius: 2 }}>note</span>}
...
function ProjectsSection({ projects, onSave, onArchive, onSyncToDay, onUnlinkSubtask, addTrigger }) {
  ...
  const addProject = () => {
    ...
    const p = { id: "p" + Date.now(), text: newName.trim(), done: false, subtasks: [] };
    onSave([...projects, p]);
    setNewName("");
    setAdding(false);
    setExpanded((e) => ({ ...e, [p.id]: true }));
    setAddingSubtask(p.id);
  };

  const deleteProject = (id) => {
    if (!window.confirm("Delete this project and all its subtasks?")) return;
  ...
  const completeProject = (id) => {
    ...
    if (!proj) return;
    if (onArchive) onArchive(proj);
    onSave(projects.filter((p) => p.id !== id));
  };

  const editProject = (id) => {
    if (!editProjText.trim()) return;
    onSave(projects.map((p) => p.id === id ? { ...p, text: editProjText.trim() } : p));
    setEditingProject(null);
  };

  const addSub = (projectId) => {
    if (!newSubtask.trim()) return;
    const sub = { id: "s" + Date.now() + "_" + Math.random().toString(36).slice(2, 6), text: newSubtask.trim(), done: false };
    onSave(projects.map((p) => p.id === projectId ? { ...p, subtasks: [...p.subtasks, sub] } : p));
    setNewSubtask("");
  };
  ...
  const handleSubDrop = (projectId, targetSubId) => {
    if (!dragSub || dragSub.projectId !== projectId) return;
    const proj = projects.find((p) => p.id === projectId);
    if (!proj) return;
    const subs = [...proj.subtasks];
    const fromIdx = subs.findIndex((s) => s.id === dragSub.subtaskId);
    if (fromIdx === -1) return;
    const [moved] = subs.splice(fromIdx, 1);
    if (targetSubId) {
      const toIdx = subs.findIndex((s) => s.id === targetSubId);
      if (toIdx !== -1) subs.splice(toIdx, 0, moved);
      else subs.push(moved);
    } else {
      subs.push(moved);
    }
    onSave(projects.map((p) => p.id === projectId ? { ...p, subtasks: subs } : p));
    setDragSub(null);
  };
      ...
      {adding && (
        <div style={{ marginBottom: 6 }}>
          <input ref={addRef} value={newName} onChange={(e) => setNewName(e.target.value)}
            ...
            style={{ width: "100%", border: "1px solid var(--border)", borderRadius: 4, padding: "4px 8px", fontSize: 12, outline: "none", background: "var(--input-bg)", color: "var(--text)", boxSizing: "border-box" }} />
        </div>
      )}
      {projects.length === 0 && !adding && (
        <div style={{ fontSize: 10, color: "var(--text-faint)", textAlign: "center", padding: "4px 0" }}>No projects yet</div>
      )}
      {projects.map((proj) => {
        const doneCount = proj.subtasks.filter((s) => s.done).length;
        const totalCount = proj.subtasks.length;
        const isExpanded = expanded[proj.id];
        return (
          <div key={proj.id} style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 6, marginBottom: 4, overflow: "hidden" }}>
            {/* Project header */}
            <div onClick={() => toggle(proj.id)} style={{ padding: "6px 8px", cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ fontSize: 8, color: "var(--text-faint)", transform: isExpanded ? "rotate(90deg)" : "rotate(0deg)", transition: "transform 0.15s", display: "inline-block" }}>{"\u25B6"}</span>
              {editingProject === proj.id ? (
                <input ref={editProjRef} value={editProjText} onClick={(e) => e.stopPropagation()}
                  onChange={(e) => setEditProjText(e.target.value)}
                  onKeyDown={(e) => { if (e.key === "Enter") editProject(proj.id); if (e.key === "Escape") setEditingProject(null); }}
                  onBlur={() => editProject(proj.id)}
                  style={{ flex: 1, border: "1px solid var(--border)", borderRadius: 3, padding: "1px 4px", fontSize: 12, outline: "none", background: "var(--input-bg)", color: "var(--text)", fontWeight: 600 }} />
              ) : (
                <span onDoubleClick={(e) => { e.stopPropagation(); setEditingProject(proj.id); setEditProjText(proj.text); }}
                  style={{ flex: 1, fontSize: 12, fontWeight: 600, color: "var(--text)", cursor: "text" }}>{proj.text}</span>
              )}
              {totalCount > 0 && (
                <span style={{ fontSize: 10, color: doneCount === totalCount ? "#6a9955" : "var(--text-muted)", fontFamily: "'JetBrains Mono', monospace", fontWeight: 600 }}>{doneCount}/{totalCount}</span>
              )}
            </div>
            {/* Expanded: subtasks */}
            {isExpanded && (
              <div style={{ padding: "0 8px 6px", borderTop: "1px solid var(--border-light)" }}>
                {proj.subtasks.map((sub) => (
                  <div key={sub.id} draggable
                    ...
                    {editingSubtask?.projectId === proj.id && editingSubtask?.subtaskId === sub.id ? (
                      <input ref={editSubRef} value={editSubText} onChange={(e) => setEditSubText(e.target.value)}
                        onKeyDown={(e) => { if (e.key === "Enter") editSub(proj.id, sub.id); if (e.key === "Escape") setEditingSubtask(null); }}
                        onBlur={() => editSub(proj.id, sub.id)}
                        style={{ flex: 1, border: "1px solid var(--border)", borderRadius: 3, padding: "1px 4px", fontSize: 11, outline: "none", background: "var(--input-bg)", color: "var(--text)" }} />
                    ) : (
                      <span onDoubleClick={() => { setEditingSubtask({ projectId: proj.id, subtaskId: sub.id }); setEditSubText(sub.text); }}
                        style={{ flex: 1, fontSize: 11, color: sub.done ? "var(--text-muted)" : "var(--text)", textDecoration: sub.done ? "line-through" : "none", cursor: "text", wordBreak: "break-word" }}>{sub.text}</span>
                    )}
                    <button onClick={() => deleteSub(proj.id, sub.id)}
                      style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-faint)", fontSize: 11, padding: 0, fontWeight: 600, flexShrink: 0 }}
                ...
                <div style={{ display: "flex", gap: 4, marginTop: 4, borderTop: "1px solid var(--border-light)", paddingTop: 4 }}>
                  ...
                  <button onClick={() => deleteProject(proj.id)}
                    ...
                    onMouseLeave={(e) => { e.target.style.borderColor = "var(--border)"; e.target.style.color = "var(--text-faint)"; }}>Delete</button>
                </div>
              </div>
            )}
          </div>
    ...
    <div ref={taskCardRef} style={{ borderLeft: task._isUpcoming ? `3px dashed var(--text-faint)` : `3px solid ${catColor}`, background: hover ? "var(--bg-hover)" : task._isUpcoming ? "transparent" : `${catColor}18`, opacity: task.done ? 0.45 : 1, transition: "opacity 0.2s, background 0.15s", marginBottom: 0, position: "relative" }}
          ...
          {task.subtasks.map((sub) => {
            ...
            <div key={sub.id} style={{ display: "flex", alignItems: "center", gap: 4, padding: "0" }}
              ...
              <div onMouseDown={(e) => { e.preventDefault(); e.stopPropagation();
                ...
                style={{ width: 12, height: 12, border: sub.done ? "1.5px solid #6a9955" : "1.5px solid var(--text-faint)", borderRadius: 2, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 8, color: sub.done ? "#6a9955" : "transparent", background: sub.done ? "rgba(106,153,85,0.15)" : "transparent", flexShrink: 0 }}>
                {sub.done && "\u2713"}
              </div>
              {editingSubId === sub.id ? (
                <input value={editSubText} onChange={(e) => setEditSubText(e.target.value)}
                  ...
                  autoFocus
                  style={{ flex: 1, border: "none", borderBottom: "1px solid var(--border)", padding: "0 2px", fontSize: 10, outline: "none", background: "transparent", color: "var(--text)", minWidth: 0 }} />
              ) : (
      ...
      {ctxMenu && (
        <div ref={(el) => {
          ...
          {task.startTime && (
            <div onClick={(e) => { onRemoveTime && onRemoveTime(columnId, task.id); setCtxMenu(null); }}
              style={{ padding: "6px 14px", cursor: "pointer", fontSize: 12, color: "var(--text)" }}
              onMouseEnter={(e) => e.currentTarget.style.background = "var(--bg-hover)"} onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>
              Remove time
            </div>
          ...
          {projects && projects.length > 0 && !task.projectId && (
            <>
              <div onClick={() => setShowProjectMenu(!showProjectMenu)}
                ...
                onMouseEnter={(e) => e.currentTarget.style.background = "var(--bg-hover)"} onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>
                <span>Assign to project</span><span style={{ fontSize: 10, color: "var(--text-faint)" }}>{"\u25B6"}</span>
              </div>
              {showProjectMenu && (
                <div style={{ padding: "2px 14px 6px", borderTop: "1px solid var(--border)" }}>
                  {projects.map((p) => (
                    <div key={p.id} onClick={() => { onAssignToProject && onAssignToProject(columnId, task.id, p.id); setCtxMenu(null); setShowProjectMenu(false); }}
                      style={{ padding: "3px 0", cursor: "pointer", fontSize: 11, color: "var(--text)" }}
                      onMouseEnter={(e) => e.currentTarget.style.color = "var(--accent)"} onMouseLeave={(e) => e.currentTarget.style.color = "var(--text)"}>{p.text}</div>
                  ))}
                </div>
              )}
            </>
          ...
          {repeatMenu && (
            <div style={{ padding: "4px 14px 8px", borderTop: "1px solid var(--border)" }}>
              ...
              {task.recurring && onSkipRecurring && (
                <div onClick={(e) => { onSkipRecurring(columnId, task.id); setCtxMenu(null); }}
                  style={{ padding: "4px 0 0", cursor: "pointer", fontSize: 10, color: "var(--text-muted)", marginTop: 2 }}>
                  Skip this week
                </div>
              )}
            </div>
    ...
    <div onDragOver={(e) => { e.preventDefault(); }}
      ...
      <div style={{
        ...
        {!isLater && onOpenJournal && <button onClick={() => onOpenJournal(dayInfo?.fullDate)} title={hasJournalEntry ? "View journal entry" : "Write journal entry"}
          style={{ background: "none", border: "none", cursor: "pointer", padding: 0, margin: "0 -2px 0 -3px", lineHeight: 1, display: "flex", alignItems: "center", flexShrink: 0, fontSize: isMobile ? 11 : 10, color: hasJournalEntry ? "var(--text-muted)" : "var(--border)", transition: "color 0.2s" }}
      ...
      <div>
        {incompleteTasks.map((task, idx) => (
          <div key={task.id}>
            ...
            <TaskCard task={task} columnId={columnId} categories={categories} onDragStart={onDragStart} onToggle={onToggle} onDelete={onDelete} onEdit={onEdit} onChangeCategory={onChangeCategory} isMobile={isMobile} onMove={onMove} onSetRecurring={onSetRecurring} onSkipRecurring={onSkipRecurring} onSetTime={onSetTime} onRemoveTime={onRemoveTime} projects={projects} onAssignToProject={onAssignToProject} onUnlink={onUnlink} onUpdateTask={onUpdateTask} highlightQuery={highlightQuery} taskFontSize={taskFontSize} />
          </div>
        ))}
        <DropZone onDrop={(e) => handleDropAtIndex(e, null)} />
        <DoneCollapse doneTasks={doneTasks} columnId={columnId} categories={categories} onDragStart={onDragStart} onToggle={onToggle} onDelete={onDelete} onEdit={onEdit} onChangeCategory={onChangeCategory} onMove={onMove} onSetRecurring={onSetRecurring} onSkipRecurring={onSkipRecurring} onSetTime={onSetTime} onRemoveTime={onRemoveTime} projects={projects} onAssignToProject={onAssignToProject} onUnlink={onUnlink} onUpdateTask={onUpdateTask} highlightQuery={highlightQuery} isMobile={isMobile} taskFontSize={taskFontSize} defaultExpanded={isReadOnly} />
      </div>
...
function UnscheduledCol({ col, untimed, done, categories, taskFontSize, toggleDone, addTask, editTask, deleteTask, changeCategory, setRecurring, setTaskTime, autoDetectCategory, handleUntimedDrop }) {
  const [showTimePicker, setShowTimePicker] = useState(false);
    ...
    <div style={{ flex: 1, minWidth: 0, borderLeft: "1px solid var(--border-light)", padding: "2px 2px" }}
      ...
      {ctxMenu && ctxTask && (
        <div data-ctx-menu="true" onMouseDown={(e) => e.stopPropagation()} onTouchStart={(e) => e.stopPropagation()} ref={(el) => { if (el) { const r = el.getBoundingClientRect(); if (r.bottom > window.innerHeight - 8) el.style.top = Math.max(8, ctxMenu.y - r.height) + "px"; if (r.right > window.innerWidth - 8) el.style.left = Math.max(8, ctxMenu.x - r.width) + "px"; } }}
          ...
          <div onClick={(e) => { setShowTimePicker(!showTimePicker); }} style={{ padding: "5px 12px", cursor: "pointer", fontSize: 11, color: "var(--text)" }} onMouseEnter={(e) => e.currentTarget.style.background = "var(--bg-hover)"} onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>Set time</div>
          {showTimePicker && (
  ...
  return (
    <>
      ...
      {expanded && doneTasks.map((task) => (
        <TaskCard key={task.id} task={task} columnId={columnId} categories={categories} onDragStart={onDragStart} onToggle={onToggle} onDelete={onDelete} onEdit={onEdit} onChangeCategory={onChangeCategory} isMobile={isMobile} onMove={onMove} onSetRecurring={onSetRecurring} onSkipRecurring={onSkipRecurring} onSetTime={onSetTime} onRemoveTime={onRemoveTime} projects={projects} onAssignToProject={onAssignToProject} onUnlink={onUnlink} onUpdateTask={onUpdateTask} highlightQuery={highlightQuery} taskFontSize={taskFontSize} />
      ))}
    </>
  );
}
    ...
    <div onDragOver={(e) => { e.preventDefault(); }}
      ...
      <div style={{ flex: 1 }}>
        {incompleteTasks.map((task, idx) => (
          <div key={task.id}>
            <DropZone onDrop={(e) => handleDropAtIndex(e, task.id)} />
            <TaskCard task={task} columnId={columnId} categories={categories} onDragStart={onDragStart} onToggle={onToggle} onDelete={onDelete} onEdit={onEdit} onChangeCategory={onChangeCategory} onMove={onMove} onSetRecurring={onSetRecurring} onSkipRecurring={onSkipRecurring} onSetTime={onSetTime} onRemoveTime={onRemoveTime} projects={projects} onAssignToProject={onAssignToProject} onUnlink={onUnlink} onUpdateTask={onUpdateTask} highlightQuery={highlightQuery} taskFontSize={taskFontSize} />
          </div>
        ))}
        <DropZone onDrop={(e) => handleDropAtIndex(e, null)} />
        <DoneCollapse doneTasks={doneTasks} columnId={columnId} categories={categories} onDragStart={onDragStart} onToggle={onToggle} onDelete={onDelete} onEdit={onEdit} onChangeCategory={onChangeCategory} onMove={onMove} onSetRecurring={onSetRecurring} onSkipRecurring={onSkipRecurring} onSetTime={onSetTime} onRemoveTime={onRemoveTime} projects={projects} onAssignToProject={onAssignToProject} onUnlink={onUnlink} onUpdateTask={onUpdateTask} highlightQuery={highlightQuery} isToday={isToday} taskFontSize={taskFontSize} defaultExpanded={isReadOnly} />
      </div>
      {adding ? (
...
export default function Planner({ data, onSave, onSaveQuiet, onSaveFuture, onSaveNotebooks, onSaveJournal, onSaveContacts, onSaveArchive, onSaveProjects, onSaveDailyHabits, onSaveWeeklyHabits, onSaveSettings, onSaveRecurringRules, onSaveMoods, onLoadWeekTasks, onSaveWeekTasks, onGetBackups, onRestoreBackup, onExportData, onLogout, userEmail, userId }) {
  ...
  const { tasks: currentTasks, futureTasks, dailyHabits, weeklyHabits, notes, habitHistory, moods, projects, recurringRules } = data;
  ...
  const viewedTasks = (() => {
    ...
    // Merge recurring task previews
    if (recurringRules && recurringRules.length > 0) {
      recurringRules.forEach((rule) => {
        const day = rule.day || "mon";
        if (!dayKeys.includes(day)) return;
        // Check skip list
        if (rule.skipWeeks && rule.skipWeeks.includes(viewedWeekKey)) return;
        // Check if rule would fire for this week
        let shouldShow = false;
        if (rule.type === "weeks") shouldShow = true; // weekly rules always show in future previews
        else if (rule.type === "until" && rule.until >= viewedWeekKey) shouldShow = true;
        else if (rule.type === "monthly") shouldShow = true; // simplified: show monthly in all future weeks
        if (!shouldShow) return;
        // Don't duplicate if task already exists
        if (merged[day].some((t) => t.text === rule.text && !t.done)) return;
        merged[day].push({ id: "rc_" + rule.text + "_" + day, text: rule.text, done: false, category: rule.category || "cat_none", _isRecurringPreview: true, recurring: rule });
      });
    }
    return merged;
  })();
  ...
  const setProjectsOpen = (v) => { setProjectsOpenState(v); try { localStorage.setItem("planner_projectsOpen", v); } catch {} };
  ...
  const skipRecurring = useCallback((col, id) => {
    ...
    // Update local state immediately so it persists
    dataRef.current = { ...dataRef.current, recurringRules: updated };
  ...
  const archiveProject = (proj) => {
    ...
    const newArchive = [entry, ...(d.archive || [])].slice(0, 500);
    update({ archive: newArchive });
    onSaveArchive(newArchive);
  };
  // Assign a day-task to a project as a linked subtask
  const assignToProject = (col, taskId, projectId) => {
    const t = dataRef.current.tasks;
    const task = (t[col] || []).find((x) => x.id === taskId);
    if (!task) return;
    const p = (dataRef.current.projects || []);
    const subId = "s" + Date.now() + "_" + Math.random().toString(36).slice(2, 6);
    // Add subtask to project
    const newProjects = p.map((proj) => proj.id === projectId ? { ...proj, subtasks: [...proj.subtasks, { id: subId, text: task.text, done: task.done }] } : proj);
  ...
  const syncToProject = (projectId, subtaskId, patch) => {
    const p = dataRef.current.projects || [];
  ...
  const wrappedDeleteTask = useCallback((col, id) => {
    ...
    // Linked task: deleting from day does NOT delete the project subtask
    deleteTask(col, id);
  }, [tasks, deleteTask]);
  ...
  const wrappedEditTask = useCallback((col, id, text) => {
    ...
    if (task && task.projectId && task.subtaskId) syncToProject(task.projectId, task.subtaskId, { text });
  }, [tasks, editTask]);
  ...
  const wrappedToggleDone = useCallback((col, id) => {
    ...
    toggleDone(col, id);
    // Sync to project if linked
    if (task && task.projectId && task.subtaskId) syncToProject(task.projectId, task.subtaskId, { done: !task.done });
  }, [tasks, toggleDone]);
                  ...
                  {(() => {
                    ...
                    const handleTimeDrop = (e, col, minuteOfDay) => {
                      ...
                      try {
                        ...
                        const duration = task?.endTime ? timeToMin(task.endTime) - timeToMin(task.startTime) : 15;
                            ...
                            {dayKeys.map((col) => {
                              ...
                              return (
                                <UnscheduledCol key={col} col={col} untimed={untimed} done={done} categories={categories} taskFontSize={taskFontSize}
                                  toggleDone={toggleDone} addTask={addTask} editTask={editTask} deleteTask={deleteTask} changeCategory={changeCategory} setRecurring={setRecurring} setTaskTime={setTaskTime} autoDetectCategory={autoDetectCategory} handleUntimedDrop={handleUntimedDrop} />
                              );
                            })}
                          </div>
                        </div>
